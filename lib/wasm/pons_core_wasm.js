 ...
 *
 * ## 사용 흐름
 * 1. `acquire_slot()` - 쓰기용 슬롯 획득 (generation 포함 반환)
 * 2. JS에서 WASM 메모리에 직접 데이터 쓰기
 * 3. `commit_slot_safe()` - generation 검증 후 헤더 생성 및 CRC 계산 (추천)
 * 4. `get_packet_view()` - WebRTC 전송용 포인터 획득
 * 5. `release_slot()` - 전송 완료 후 슬롯 반환
 *
 * ## 안전성 (Generation Guard)
 * JS가 이미 반환된 슬롯에 데이터를 쓰려고 하면, generation이 일치하지 않아
 * commit이 거부됩니다 (Use-after-free 방지)
 */
export class ZeroCopyPacketPool {
     ...
     * @returns {number}
     */
    commit_slot_safe(slot_id, data_len, generation) {
        const ret = wasm.zerocopypacketpool_commit_slot_safe(this.__wbg_ptr, slot_id, data_len, generation);
        return ret >>> 0;
    }
    /**
     * 🚀 [신규] 암호화 패킷 커밋
...
export class Zip64Stream {
    ...
    process_chunk(data) {
        try {
            ...
            const ptr0 = passArray8ToWasm0(data, wasm.__wbindgen_export);
            const len0 = WASM_VECTOR_LEN;
            wasm.zip64stream_process_chunk(retptr, this.__wbg_ptr, ptr0, len0);
            var r0 = getDataViewMemory0().getInt32(retptr + 4 * 0, true);
            var r1 = getDataViewMemory0().getInt32(retptr + 4 * 1, true);
            var v2 = getArrayU8FromWasm0(r0, r1).slice();
            wasm.__wbindgen_export3(r0, r1 * 1, 1);
