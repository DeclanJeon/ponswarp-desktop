...
class RustSignalingAdapter {
  ...
  private handleMessage(message: RustMessage) {
    ...
    if (typeof payload === 'object' && payload !== null) {
      ...
      if (message.type === 'Manifest') {
        console.log('[RustSignaling] 📦 Manifest Message Received');

        // payload 구조가 { from: "...", manifest: "..." } 인지 확인
        if (payloadObj.manifest && typeof payloadObj.manifest === 'string') {
          try {
            // 디버깅용: 미리 파싱해봄
            const parsed = JSON.parse(payloadObj.manifest as string);
            console.log('[RustSignaling] 📦 Manifest Preview:', parsed);
          } catch (e) {
            console.warn(
              '[RustSignaling] ⚠️ Manifest string parsing check failed (might be raw string):',
              e
            );
          }
        } else {
          console.warn(
            '[RustSignaling] ⚠️ Manifest payload missing "manifest" string field:',
            payloadObj
          );
        }
      }
      ...
      // 🆕 TransferComplete 디버깅
      if (message.type === 'TransferComplete') {
        console.log(
          '[RustSignaling] ✅ TransferComplete received from:',
          payloadObj.from
        );
      }
    }
  ...
  /**
   * 🆕 전송 완료 신호 (Receiver -> Sender)
   * Receiver가 파일 수신을 완료했음을 Sender에게 알립니다.
   */
