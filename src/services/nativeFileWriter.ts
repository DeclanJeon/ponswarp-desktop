...
export class NativeFileWriter {
  ...
  public async initStorage(manifest: any): Promise<void> {
    ...
    logInfo(
      '[NativeWriter]',
      `Initializing for ${fileCount} files. Total: ${(manifest.totalSize / 1024 / 1024).toFixed(2)} MB`
    );
  ...
  private async processChunkInternal(packet: ArrayBuffer): Promise<void> {
    ...
    // const _offset = Number(view.getBigUint64(6, true)); // 64-bit offset
    ...
    if (fileIndex !== this.currentFileIndex) {
      logDebug(
        '[NativeWriter]',
        `File Switch: ${this.currentFileIndex} -> ${fileIndex}`
      );
  ...
  private async switchFile(newIndex: number): Promise<void> {
    ...
    logInfo(
      '[NativeWriter]',
      `Opening file: ${fileNode.path} (${(fileNode.size / 1024).toFixed(1)} KB)`
    );
  ...
  private async finalize(): Promise<void> {
    ...
    logInfo(
      '[NativeWriter]',
      `Transfer Complete! Total written: ${this.totalBytesWritten} bytes`
    );
    this.onCompleteCallback?.(this.totalBytesWritten);
  }

  // --- Public Listeners ---
  public onProgress(cb: any) {
    this.onProgressCallback = cb;
  }
  public onComplete(cb: any) {
    this.onCompleteCallback = cb;
  }
  public onError(cb: any) {
    this.onErrorCallback = cb;
  }
  public onFlowControl(cb: any) {
    this.onFlowControlCallback = cb;
  }
  public setEncryptionKey() {
    /* Native handles crypto in Rust if needed */
  }
