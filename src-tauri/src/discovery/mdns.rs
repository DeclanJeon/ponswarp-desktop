...
impl DiscoveryService {
    pub fn new(node_id: String, port: u16) -> Result<Self> {
        let daemon = ServiceDaemon::new().map_err(|e| anyhow::anyhow!("mDNS 데몬 생성 실패: {}", e))?;
    ...
    pub fn register(&self) -> Result<()> {
        ...
        self.daemon.register(service).map_err(|e| anyhow::anyhow!("mDNS 등록 실패: {}", e))?;
                ...
                match receiver.recv_timeout(std::time::Duration::from_secs(1)) {
                    Ok(event) => match event {
                        ServiceEvent::ServiceResolved(info) => {
                            ...
                            let selected_addr = Self::select_best_address_static(&addrs, info.get_port());
                            
                            if let Some(socket_addr) = selected_addr {
                                info!("🔍 [DEBUG] Selected address: {} for peer {}", socket_addr, peer_id);
    ...
    /// 🆕 여러 주소 중에서 최적의 주소 선택
    fn select_best_address_static(addrs: &std::collections::HashSet<std::net::IpAddr>, port: u16) -> Option<SocketAddr> {
        use std::net::IpAddr;
